...
namespace ShrimpFlourControl.Vehicles
{
    public class AGVHandler
    {
        ...
        public void SendAGVTo(Node departNode, Node destinateNode, AGV selectAGV) // 車輛派遣
        {
            if (departNode != null)
            {
                if (departNode == null)
                {
                    //departNode = currentSelectedNode;
                }
                else if (destinateNode == null)
                {
                    //destinateNode = currentSelectedNode;
                    var routePath = PathPlanner.FindPath(departNode, destinateNode);
                    if (routePath != null)
                    {
                    ...
                    {
                        MessageBox.Show("找不到路徑", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
            }
        }
